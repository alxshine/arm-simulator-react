...
export function logical_shift_right(input: Uint32, shift_amount: ShiftAmount): BarrelShifterReturn {
  const actual_shift_amount = shift_amount.value === 0 ? 32 : shift_amount.value
...
export function logical_shift_left(input: Uint32, shift_amount: ShiftAmount): BarrelShifterReturn {
  const actual_shift_amount = shift_amount.value === 0 ? 32 : shift_amount.value
...
export function arithmetic_shift_right(input: Uint32, shift_amount: ShiftAmount): BarrelShifterReturn {
  const actual_shift_amount = shift_amount.value === 0 ? 32 : shift_amount.value
  const carry = (input.value & (1 << (actual_shift_amount - 1))) !== 0
  const carry_affected = actual_shift_amount !== 0
  let result = input.value >> actual_shift_amount
  if (input.value & 0x80000000) {
    const mask = (0xffffffff >> (32 - actual_shift_amount)) << (32 - actual_shift_amount)
    result |= mask
  }
  const negative = (result & 0x80000000) !== 0
  const zero = result === 0
...
export function rotate_right(input: Uint32, shift_amount: ShiftAmount): BarrelShifterReturn {
  const actual_shift_amount = shift_amount.value === 0 ? 32 : shift_amount.value
  const carry = (input.value & (1 << (actual_shift_amount - 1))) !== 0
  const carry_affected = actual_shift_amount !== 0
  const result = (input.value >> actual_shift_amount) | (input.value << (32 - actual_shift_amount))
  const negative = (result & 0x80000000) !== 0
  const zero = result === 0
...
export function execute_shift_config(input: Uint32, shift_config: ShiftConfiguration): BarrelShifterReturn {
  switch (+shift_config) {
